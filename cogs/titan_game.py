...
class TitanGameCog(commands.Cog):
    ...
    @commands.hybrid_group(name="titan_game", description="Titan Shifters Social Deduction Game", invoke_without_command=True)
    ...
    async def tg_start(self, ctx: commands.Context):
        ...
        await ctx.send("Starting game... Check your DMs for your roles!")
        ...
        if "Transformation" in TITAN_IMAGES:
            embed.set_image(url=random.choice(TITAN_IMAGES["Transformation"]))
    ...
    async def eliminate(self, ctx: commands.Context, target: discord.User):
        ...
        if not isinstance(ctx.channel, discord.DMChannel) and not ctx.interaction:
            ...
            try:
                ...
                pass
            await ctx.author.send("⚠️ You just used !eliminate in public! Be careful, you might have exposed yourself.")
    ...
    async def meeting(self, ctx: commands.Context):
        ...
        if game.call_meeting(ctx.author.id):
            ...
            await ctx.send(embed=embed)
            game.start_voting()
            await ctx.channel.send("🗣️ **Discussion & Voting Phase started!** Use `!vote @user` or `/vote @user` to cast your vote. To skip voting, use `!vote` without a mention.")
        else:
    ...
    async def end_game(self, channel: discord.TextChannel, game: TitanGameEngine, winner: Role):
        ...
        if winner == Role.SURVEY_CORPS:
            ...
            if "Founding Titan" in TITAN_IMAGES:
                 embed.set_image(url=random.choice(TITAN_IMAGES["Founding Titan"]))
        else:
            embed.description = "💀 **The Titans have overrun the Survey Corps! Humanity falls.**"
            if "Pure Titan" in TITAN_IMAGES:
                 embed.set_image(url=random.choice(TITAN_IMAGES["Pure Titan"]))
