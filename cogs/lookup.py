...
NUMBER_VERIFY_URL = "https://apilayer.net/api/validate"
...
class Lookup(commands.Cog):
    ...
    async def _fetch_number_info(self, phone: str) -> discord.Embed:
        ...
        params = {
            "access_key": NUMBER_VERIFY_API_KEY,
            "number": phone,
            "country_code": "",
            "format": "1"
        }
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(NUMBER_VERIFY_URL, params=params, timeout=aiohttp.ClientTimeout(total=10)) as resp:
                    if resp.status != 200:
        ...
        # APILayer returns {"success": false, "error": {...}} on auth errors
        if not data.get("valid") and "error" in data:
            err = data["error"]
            return self._error_embed(f"{err.get('type', 'Error')} — {err.get('info', 'Unknown error')}")
        ...
        embed.set_footer(text="Powered by APILayer Number Verification API")
        return embed
