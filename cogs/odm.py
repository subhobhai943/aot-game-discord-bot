...
class ODM(commands.Cog):
    ...
    async def odm_grapple(self, interaction: discord.Interaction, distance: int, speed: str = "normal", gas: int = 100):
        ...
        if not (10 <= distance <= 200):
            await interaction.response.send_message("\u274c Distance must be 10-200m.", ephemeral=True)
            return
        gear = ODMGear(gas_capacity=float(gas), blade_durability=100)
        result = gear.grapple(distance_m=float(distance), speed=speed)
        embed = discord.Embed(title="\U0001fa82 ODM Gear Grapple", color=discord.Color.teal())
    ...
    async def odm_strike(self, interaction: discord.Interaction, armor_level: int = 0, abilities: str = ""):
        ...
        embed = discord.Embed(title="\U0001fa7a Nape Strike", color=discord.Color.dark_red())
        embed.add_field(name="Armor Level", value=armor_level, inline=True)
        embed.add_field(name="Abilities", value=abilities or "None", inline=True)
