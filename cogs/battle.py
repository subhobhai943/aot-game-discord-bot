...
HIT_LINES = [
    "The nape is exposed! ⚡", "A clean cut through the hardened skin!",
    "Blades find their mark! \U0001f5e1\ufe0f", "The Survey Corps training pays off!",
    "ODM gear whirs as they close in fast!", "The titan staggers backward!",
...
MISS_LINES = [
    ...
    "Too slow — the titan swipes them away!", "The ODM wire snaps at the worst moment!",
    "A stumble on the rooftop — the hit misses!", "Crystal armor absorbs the blow!",
...
def _hp_bar(current: int, maximum: int, length: int = 12) -> str:
    ...
    if pct > 0.6:
        ...
        bar_char = "\U0001f7e8"  # yellow square
    else:
...
# ── Active battle state tracker (in-memory, per user) ───────────────────
class LiveBattle:
    """Extended battle state with combo, crits, status effects."""
    __slots__ = (
    ...
    def __init__(self, user_id, username, scout_name, titan_name):
        ...
        self.round_num    = 1
        self.active       = True
        self.combo        = 0      # consecutive non-miss hits
        self.scout_status: str | None = None   # BURN / STUN / SHIELD / RAGE
        self.titan_status: str | None = None
        self.last_move    = ""
        self.last_used    = 0.0    # epoch for cooldown
        self.xp_earned    = 0
        self.coins_earned = 0
        self.channel_id   = 0
        self.message_id   = 0
...
class BattleView(discord.ui.View):
    ...
    def _add_buttons(self):
        ...
        for move_key in ["spiral_cut", "titan_smash", "defend"]:
            ...
            btn.callback = self._make_callback(move_key)
            self.add_item(btn)
        # Row 2 — RAGE (unlocks at combo >= 3) + Flee
    ...
    def _make_callback(self, move_key: str):
        async def callback(interaction: discord.Interaction):
            if str(interaction.user.id) != self.battle.user_id:
                ...
                return
            # Cooldown check — 3 seconds between moves
            now = time.time()
    ...
    async def _flee_callback(self, interaction: discord.Interaction):
        ...
        await interaction.response.edit_message(
            content=f"\U0001f3c3 **{self.battle.username}** fled from **{self.battle.titan_name}**! Coward or survivor? \U0001f914",
...
def _status_bar(battle: LiveBattle) -> str:
    ...
    if battle.combo >= 2:
        parts.append(f"\U0001f525 **Combo x{battle.combo}** {'\u26a1' * min(battle.combo, 5)}")
    return "  \u2502  ".join(parts) if parts else "No active effects"


def _build_battle_embed(battle: LiveBattle, gif_url: str,
                        round_log: list[str], title_override: str = "") -> discord.Embed:
    t_stats = TITAN_STATS.get(battle.titan_name, {})
    rarity  = t_stats.get("rarity", "Common")
    color   = RARITY_COLOR.get(rarity, 0x5599FF)

    # Urgency color override
    if battle.scout_hp / battle.scout_max_hp < 0.25:
        color = 0xFF0000  # critical red when near death

    title = title_override or (
        f"\u2694\ufe0f {battle.scout_name} vs {battle.titan_name} — Round {battle.round_num}"
    )
    ...
    embed.set_footer(
        text=f"\U0001f9e0 Combo: {battle.combo}  |  "
             f"\U0001f985 Wings of Freedom  |  "
             f"Rage unlocks at Combo x3"
    )
    return embed
...
class Battle(commands.Cog):
    ...
    @app_commands.choices(
        ...
        titan=[
            app_commands.Choice(name=t, value=t) for t in TITAN_STATS
        ]
    )
    async def fight_slash(self, interaction: discord.Interaction, character: str, titan: str):
        ...
        if uid in _BATTLES and _BATTLES[uid].active:
            ...
            return
        await interaction.response.defer()
        await self._start_battle(interaction.followup.send, uid,
    ...
    async def fight_prefix(self, ctx: commands.Context, *, args: str = ""):
        ...
        if " vs " not in args:
            ...
            return
        parts, c_raw, t_raw = args.split(" vs ", 1), *args.split(" vs ", 1)
        c_raw = args.split(" vs ", 1)[0].strip().title()
        t_raw = args.split(" vs ", 1)[1].strip().title()
    ...
    async def flee(self, ctx: commands.Context):
        ...
        await ctx.send(
            ...
            f"Coward or survivor? \U0001f914"
        )

    # ── Battle starter ─────────────────────────────────────────────────────────────
    async def _start_battle(self, send_fn, uid: str, username: str,
                            character: str, titan: str):
        battle = LiveBattle(uid, username, character, titan)
        _BATTLES[uid] = battle

        gif_url = await get_gif("salute")  # opening cinematic
        ...
        embed = discord.Embed(
            ...
            description=(
                f"> *\"If you win, you live. If you lose, you die. "
                f"If you don\'t fight, you can\'t win!\"*\n"
                f"> \u2014 Eren Yeager"
            ),
            color=RARITY_COLOR.get(rarity, 0x5599FF)
        )
    ...
    # ── Core move processor ─────────────────────────────────────────────────────
    async def process_move(self, interaction: discord.Interaction,
                           battle: LiveBattle, move_key: str):
        ...
        if battle.scout_status == "STUN":
            ...
            if move_key == "defend":
                ...
                battle.combo = 0   # defend resets combo
                log.append(f"\U0001f6e1\ufe0f **{battle.scout_name}** raises their blades in defence! **SHIELD** active.")
                ...
                if missed:
                    battle.combo = 0
                    log.append(f"\U0001f4a8 **{battle.scout_name}** {random.choice(MISS_LINES)}")
                    gif_url = ""
                else:
                    lo, hi = meta["dmg"]
                    ...
                    if battle.combo >= 2:
                        ...
                        dmg += combo_bonus
                        log.append(f"\U0001f525 **Combo x{battle.combo}!** +{combo_bonus} bonus damage!")
                    # Crit check
                    ...
                    if is_crit:
                        ...
                        log.append(f"\u2705 **{battle.scout_name}** {random.choice(HIT_LINES)} **−{dmg} DMG**")
                    # Apply titan status effect
                    if meta["status_apply"] and random.random() < 0.35:
                        battle.titan_status = meta["status_apply"]
                        s = STATUS[meta["status_apply"]]
                        log.append(f"{s['emoji']} Titan is now **{meta['status_apply']}**! {s['desc']}")
        ...
        if battle.titan_status == "BURN":
            burn_dmg = 15
            battle.titan_hp = max(0, battle.titan_hp - burn_dmg)
            log.append(f"\U0001f525 Titan takes **{burn_dmg} burn damage!**")
            battle.titan_status = None  # burn consumed
        ...
        if battle.titan_status != "STUN":
            ...
            if battle.scout_status == "SHIELD":
                battle.scout_status = None
                log.append(f"\U0001f6e1\ufe0f **SHIELD BLOCKED** the titan's attack! No damage taken.")
                gif_url = gif_url or await get_gif(TITAN_GIF.get(battle.titan_name, "titan_eat"))
            elif t_missed:
                log.append(f"\U0001f4a8 {battle.titan_name} {t_desc} — **MISSED!**")
            else:
                battle.scout_hp = max(0, battle.scout_hp - t_dmg)
                log.append(f"\U0001f534 {battle.titan_name} {random.choice(TITAN_HIT_LINES)} **−{t_dmg} DMG**")
                if not gif_url:
    ...
    async def _handle_victory(self, interaction: discord.Interaction,
                               battle: LiveBattle, log: list[str]):
        battle.active = False
        ...
        view.add_item(discord.ui.Button(
            label="\U0001f504 Battle Again", style=discord.ButtonStyle.green,
            custom_id="battle_again", disabled=True
        ))
        await interaction.edit_original_response(embed=embed, view=view)
    ...
    async def _handle_defeat(self, interaction: discord.Interaction,
        ...
        player = await GameState.get_player(battle.user_id, battle.username)
        player.losses += 1
        player.add_xp(20)  # consolation XP
        await GameState.save_player(player)
